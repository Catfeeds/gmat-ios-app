...
- (void)didMoveToSuperview {
	...
	[self.detailSwitch mas_updateConstraints:^(MASConstraintMaker *make) {
		make.right.mas_equalTo(- 15);
		make.centerY.mas_equalTo(self);
		make.left.mas_greaterThanOrEqualTo(self.titleNameLabel.mas_right).offset(10).priority(900);
	}];
...
- (void)setModel:(THMinePreferenceModel *)model row:(NSInteger)row {
    ...
    if (model.modelStyle == THMinePreferenceModelStyleTitleDetail) {
        ...
        self.headImageView.hidden = self.detailSwitch.hidden = true;
		self.detailNameLabel.hidden = false;
		...
		self.detailSwitch.hidden = true;
        self.headImageView.hidden = false;
		self.detailNameLabel.hidden = true;
        self.titleNameLabel.font = [[UIFont ht_fontStyle:HTFontStyleTitleLarge] ht_userSizeFont];
        self.titleNameLabel.textColor = [UIColor ht_colorStyle:HTColorStylePrimaryTitle];
        self.backgroundColor = [UIColor ht_colorStyle:HTColorStyleBackground];
        modelHeight = HTADAPT568(45);
	...
	} else if (model.modelStyle == THMinePreferenceModelStyleRightSwitch) {
		self.selectionStyle = UITableViewCellSelectionStyleNone;
		self.detailSwitch.hidden = false;
		self.headImageView.hidden = true;
		self.detailNameLabel.hidden = true;
		self.titleNameLabel.font = [[UIFont ht_fontStyle:HTFontStyleTitleLarge] ht_userSizeFont];
		self.titleNameLabel.textColor = [UIColor ht_colorStyle:HTColorStylePrimaryTitle];
		self.backgroundColor = [UIColor ht_colorStyle:HTColorStyleBackground];
		[self.detailSwitch setOn:model.detailName.boolValue animated:false];
		[self.detailSwitch bk_addEventHandler:^(id sender) {
			if (model.everyOneStyle == THMinePreferenceModelEveryOneVideoNotWiFI) {
				[HTVideoWiFIManager setAllowNotWiFiPlayVideo:weakSelf.detailSwitch.on];
			}
