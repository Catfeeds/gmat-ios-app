...
//			replyAttributedString.lineBreakMode = NSLineBreakByTruncatingTail;
		...
		//帖子内容高度
		layoutModel.totalHeightForLive = layoutModel.titleNameLabelHeight + layoutModel.detailNameLabelHeight + layoutModel.imageCollectionViewHeight + CommunityLikeReplyViewHeight + CommunityUserViewHeight;
		layoutModel.singleLiveReplyHeightArray = [NSMutableArray array];
		
		
		//帖子第一层回复高度 heigth
		[layoutModel.originModel.reply enumerateObjectsUsingBlock:^(CommunityReply * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
			NSString *str = [NSString stringWithFormat:@"%@: %@",obj.uName,obj.content];
		CGRect rec = [str boundingRectWithSize:CGSizeMake(HTSCREENWIDTH - 20, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:14]} context:nil];
			CGFloat height = ceil(10 + rec.size.height + 20 +  15 + 10); //height:第一层回复高度: 顶部距离+文字高度+中间间距 + 按钮高度 + 底部距离
			[layoutModel.singleLiveReplyHeightArray addObject:@(height).stringValue];
			
			//最多显示5条
			if (idx < 5) {
				layoutModel.totalHeightForLive += height;
			}
