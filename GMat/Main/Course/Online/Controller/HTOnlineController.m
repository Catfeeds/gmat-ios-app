...
static NSString *onlineHeaderCourse = @"508";

@implementation HTOnlineController
...
- (void)initializeDataSource {
    __weak HTOnlineController *weakSelf = self;
	[self.collectionView ht_setRefreshBlock:^(NSString *pageSize, NSString *currentPage) {
		HTNetworkModel *networkModel = [HTNetworkModel modelForOnlyCacheNoInterfaceForScrollViewWithCacheStyle:HTCacheStyleAllUser];
		[HTRequestManager requestLiveCourseWithNetworkModel:networkModel complete:^(id response, HTError *errorModel) {
			...
			if (currentPage.integerValue == 1) {
				...
				[modelArray enumerateObjectsUsingBlock:^(HTCourseOnlineVideoModel *obj, NSUInteger idx, BOOL * _Nonnull stop) {
					if ([obj.contentid isEqualToString:onlineHeaderCourse]) {
