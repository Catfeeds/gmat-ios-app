...
- (UIButton *)analysisButton
{
	if (!_analysisButton) {
		...
		[_analysisButton setTitle:@"关闭解析" forState:UIControlStateSelected];
		[_analysisButton setBackgroundColor:[UIColor ht_colorString:@"718cb4"]];
		_analysisButton.titleLabel.font = [UIFont systemFontOfSize:15];
...
{
	if (!_discussButton) {
		_discussButton = [[UIButton alloc] init];
		[_discussButton setTitle:@"参与讨论" forState:UIControlStateNormal];
		[_discussButton setBackgroundColor:[UIColor ht_colorString:@"718cb4"]];
		_discussButton.titleLabel.font = [UIFont systemFontOfSize:15];
	}
...
- (NSArray<HTQuestionMoreMenuModel *> *)moreModelArray {
	if (!_moreModelArray) {
		_moreModelArray = [HTQuestionMoreMenuModel packModelArrayIsHiddenAnalysis:self.isHiddenAnalysis];
	}
