...
- (void)requestQuestionDetailModel {
	...
	requestQuestionModelStatus = ^(HTQuestionModel *questionModel, id originResponse) {
		...
		weakSelf.questionViewGroup.analysisButton.selected = NO;
		...
		void(^autoExerciseRobot)(void) = ^() {
			...
			dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(autoExerciseTime * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
				...
				if (answerSelectedTabelView && weakSelf.blockPackage.questionSubmitBlock) {
					...
					[weakSelf.questionViewGroup.bottomSureButton ht_responderTap];
				}
			});
		};
...
- (void)initializeUserInterface {
	...
	if (self.blockPackage.questionSubmitBlock) {
		self.questionViewGroup.isHiddenAnalysis = YES;
		[self.view addSubview:self.questionViewGroup.analysisButton];
		[self.view addSubview:self.questionViewGroup.discussButton];
		
		
		[self.questionViewGroup.analysisButton mas_updateConstraints:^(MASConstraintMaker *make) {
			make.height.mas_equalTo(49);
			make.width.mas_equalTo(100.0/375.0*(HTSCREENWIDTH-2));
			make.bottom.mas_equalTo(self.view);
			make.left.mas_equalTo(self.view);
		
		}];
		[self.questionViewGroup.discussButton mas_updateConstraints:^(MASConstraintMaker *make) {
			make.bottom.mas_equalTo(self.view);
			make.width.mas_equalTo(self.questionViewGroup.analysisButton);
			make.left.mas_equalTo(self.questionViewGroup.analysisButton.mas_right).offset(1);
			make.height.mas_equalTo(self.questionViewGroup.analysisButton);
		}];
		
		
		[self.questionViewGroup.bottomSureButton mas_updateConstraints:^(MASConstraintMaker *make) {
			make.bottom.mas_equalTo(self.view);
			make.left.mas_equalTo(self.questionViewGroup.discussButton.mas_right).offset(1);
		...
		self.questionViewGroup.bottomSureButton.hidden = true;
		self.questionViewGroup.isHiddenAnalysis = NO;
	}
...
//    self.questionViewGroup.bottomSureButton.hidden = self.blockPackage.questionSubmitBlock ? false : true;
	...
	[self.questionViewGroup.analysisButton ht_whenTap:^(UIView *view) {
		BOOL isSelected = !weakSelf.questionViewGroup.analysisButton.selected;
		weakSelf.questionViewGroup.analysisButton.selected =isSelected ;
		weakSelf.questionViewGroup.questionView.showUserExerciseDuration = false;
		weakSelf.questionViewGroup.questionView.showUserAnswerParse = isSelected;
	...
	[self.questionViewGroup.discussButton ht_whenTap:^(UIView *view) {
		HTDiscussController *discussController = [[HTDiscussController alloc] init];
		discussController.questionIdString = weakSelf.questionModel.questionId;
		[weakSelf.navigationController pushViewController:discussController animated:true];
	}];
...
- (HTQuestionViewGroup *)questionViewGroup {
	if (!_questionViewGroup) {
		...
		_questionViewGroup.isHiddenAnalysis = YES;
	}
