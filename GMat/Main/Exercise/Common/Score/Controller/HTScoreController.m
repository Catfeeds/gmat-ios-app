...
- (void)initializeUserInterface {
	...
	self.navigationItem.title = @"雷哥GMAT-答题结果";
	...
	self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] bk_initWithImage:[[UIImage imageNamed:@"Toeflshare"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate] style:UIBarButtonItemStylePlain handler:^(id sender) {
		...
		[THShareView showTitle:@"雷哥GMAT-答题结果" detail:@"我在雷哥 GMAT 的答题结果" image:shotScreentImage url:GmatResourse(@"") type:SSDKContentTypeImage];
	}];
			 ...
			 .rowHeight(60) didSelectedCellBlock:^(UITableView *tableView, NSInteger row, UITableViewCell *cell, id model) {
                if (row == 0) {
                    ...
                    [HTAlert title:@"警告" message:@"重新做题之后, 之前的记录会被清除" sureAction:^{
						NSString *tempContinue = [[NSUserDefaults standardUserDefaults]objectForKey:@"tempContinue"];
						[[NSUserDefaults standardUserDefaults] setObject:tempContinue forKey:HTPlaceholderString([HTUserManager currentUser].uid,@"0")];
