...
- (void)didSelectedExerciseCell:(HTExerciseCell *)cell row:(NSInteger)row model:(HTExerciseModel *)model normalLookScore:(BOOL)normalLookScore {
	...
	NSString *continueKey = nil; //用于标记 "继续icon"
	...
	if (continueKey) {
		[[NSUserDefaults standardUserDefaults]setObject:continueKey forKey:HTPlaceholderString([HTUserManager currentUser].uid,@"0")];
	}
