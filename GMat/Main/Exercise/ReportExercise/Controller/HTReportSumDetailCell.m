...
@interface HTReportSumDetailCell ()

@property (nonatomic, strong) UITextView *textView;

@end

@implementation HTReportSumDetailCell
...
- (void)didMoveToSuperview {
	...
	[self.textView mas_makeConstraints:^(MASConstraintMaker *make) {
		make.top.mas_equalTo(self.segmentedControl.mas_bottom).offset(15);
		make.left.mas_equalTo(self).offset(15);
		make.right.mas_equalTo(self).offset(- 15);
	}];
}
...
- (void)setModel:(HTReportModel *)model row:(NSInteger)row {
    ...
    self.textView.ht_h = [self.textView.text ht_stringHeightWithWidth:HTSCREENWIDTH - 30 font:self.textView.font textView:self.textView];
	[self.textView mas_updateConstraints:^(MASConstraintMaker *make) {
		make.height.mas_equalTo(self.textView.ht_h);
	}];
    CGFloat modelHeight = self.textView.ht_h + 30 + 30;
    [model ht_setRowHeightNumber:@(modelHeight) forCellClass:self.class];
}

- (UISegmentedControl *)segmentedControl {
	if (!_segmentedControl) {
		_segmentedControl = [[UISegmentedControl alloc] initWithItems:@[@"SC", @"RC", @"CR", @"Q"]];
	}
