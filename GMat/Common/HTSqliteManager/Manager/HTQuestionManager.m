...
+ (NSString *)questionManagerSqlitePath {
    NSString *entryFilePath = [[NSBundle mainBundle] pathForAuxiliaryExecutable:@"gmat.13"];
    NSString *detryFilePath = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, true) lastObject] stringByAppendingPathComponent:@"gmat_13.sqlite"];
...
+ (NSString *)getUserAnswerWithQuestionID:(NSString *)questionID{
	...
	NSString *userAnserString = nil;
	
	FMDatabase *database = [FMDatabase databaseWithPath:[HTUserManager userManagerSqlitePath]];
	if ([database open]) {
		FMResultSet *exerciseRecordResultSet = [database executeQuery:[NSString stringWithFormat:@"select * from %@ where  userid = ? and exerciseState = 1 and questionId = ?", kuserExerciseRecordTableName], [HTUserManager currentUser].userid,questionID];
		while (exerciseRecordResultSet.next) {
			
			userAnserString= [exerciseRecordResultSet stringForColumn:@"questionUserAnswer"];
		}
		[database close];
	}
	return userAnserString;
}
...
//select * from "kuserExerciseRecordTableName" where date(CURRENT_TIMESTAMP,'localtime') =  strftime('%Y-%m-%d',questionSubmitTime,'unixepoch','localtime')

+ (NSString *)sumUserTodayExerciseRecordCount {
	FMDatabase *recordDataBase = [FMDatabase databaseWithPath:[HTUserManager userManagerSqlitePath]];
	
	if ([recordDataBase open]) {
		FMResultSet *userRecordSet = [recordDataBase executeQuery:[NSString stringWithFormat:@"select count(*) as sum from %@ where date(CURRENT_TIMESTAMP,'localtime') =  strftime('%%Y-%%m-%%d',questionSubmitTime,'unixepoch','localtime') and userid=?", kuserExerciseRecordTableName], [HTUserManager currentUser].userid];
		while (userRecordSet.next) {
//			[recordResultSet stringForColumn:@"questionId"]
		}
		[recordDataBase close];
	}
	return nil;
...
+ (NSString *)localSqliteLastUpdateTime {
	...
	localSqliteLastUpdateTime = HTPlaceholderString(localSqliteLastUpdateTime, @"1509958060");
